"""
Batch script for 3D mean fMRI normalization using global z-score normalization.
...
input_dir = '/path/to/data/fMRI_mean'
output_dir = '/path/to/data/fMRI_mean_norm'
...
def process_single_file(fmri_file_path, output_dir, idx, total):
    """
    Process a single 3D mean fMRI file: smooth, mask, normalize, and save.
    
    Args:
        fmri_file_path: Path to input 3D mean fMRI file
    ...
    Returns:
        tuple: (success: bool, filename: str, error_message: str or None, status: str)
    """
    ...
    try:
        ...
        # Step 3: Normalize globally (z-score across all voxels)
        ...
        normalized[mask_data == 0] = 0  # Force background to 0
...
print(f"Found {len(fmri_files)} 3D mean fMRI files to process")
